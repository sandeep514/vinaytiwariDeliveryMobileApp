...
const AddQty = ({
  ...
  }, [data, savedSalePrice, selectedItemFromItemsScreen]);
  ...
  function updateTotalPrice() {
    ...
    for (let i = 0; i < newData.length; i++) {
      if (Object.values(newData[i])[0].VATstatus == true) {
        price =
          price +
          Object.values(newData[i])[0].sale_price *
            Object.values(newData[i])[0].order_qty *
            1.2;
      } else {
        price =
          price +
    ...
    <View
      ...
      <View key={generateRandString()} style={styles.inputBox}>
        <TextInput
          ...
          placeholder="Qty"
          defaultValue={(val?.order_qty).toString()}
          ref={value => {}}
        ...
        <TextInput
          keyboardType="numeric"
          placeholder="Price"
          defaultValue={(Object.keys(beforeUpdPrice).length > 0 &&
