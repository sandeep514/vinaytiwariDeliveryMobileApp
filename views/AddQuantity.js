...
import {getCartItemDetails, imagePrefix} from '../api/apiService';
...
export default function AddQuantity({route, navigation}) {
  ...
  let undeliveredItems = {
    'LOAD-20-03-2021-246__134': {value: 5, cardId: 134, VATstatus: false},
    'LOAD-20-03-2021-246__311': {value: 6, cardId: 311, VATstatus: false},
  };
  ...
  async function selectedLoadedItemsByQty() {
    // cache.store('undeliveredItems', undeliveredItems);
    setLoadedActivityIndicator(true);

    cache.get('VATStatus').then(data => {
      if (data == 'true') {
        cache.store('currentVATstatus', '1');
      } else {
        cache.store('currentVATstatus', '0');
      }
      setVATstatus(data);
    });

    setLoadedData(route?.params?.data);
    try {
      await getCartItemDetails(route?.params?.data).then(res => {
        ...
        for (let i = 0; i < productData.length; i++) {
          ...
          if (myNewData['VATstatus'] == true) {
            price =
              price + myNewData['sale_price'] * myNewData['order_qty'] * 1.2;
          } else {
    ...
    cache.get('selectedVehicleNo').then(vehNo => {
      selectedVehicle = vehNo;
      cache.get('user_id').then(driverId => {
        selectedDriver = driverId;
        cache.get('selectedRoute').then(route => {
          selectedRoute = route;
  ...
  function SaveOrders() {
    ...
    if (IsKeyboardOpen) {
      ...
      // cache.store('finalItems', setUpdatedDataArray);
      updateRecords(setUpdatedDataArray).then(res => {
        let data = [];
        data.push(res);
        data.push({type: creaditStatus});
        cache.get('currentVATstatus').then(VATstatus => {
          // data.push({'has_vat' : parseInt(VATstatus)});
          cache.store('readyForOrder', data);
  ...
  function updateVATStatusOfProduct(dnum, itemId) {
    ...
    if (dnum + '__' + itemId in objectData) {
      ...
      // cache.store('itemsForVAT', VATUpdatedStatus);
    ...
    for (let i = 0; i < myData.length; i++) {
      if (myData[i][dnum] != undefined) {
        ...
        if (myData[i][dnum]['VATstatus'] == true) {
          price =
            price +
            myData[i][dnum]['sale_price'] * myData[i][dnum]['order_qty'] * 1.2;
        } else {
          price =
            price +
            myData[i][dnum]['sale_price'] * myData[i][dnum]['order_qty'];
        }
      }
    }
      ...
      if (Object.values(myData[i]).length > 0) {
        if (Object.values(myData[i])[0] != undefined) {
          if (Object.values(myData[i])[0]['VATstatus'] == true) {
            price =
              price +
              Object.values(myData[i])[0]['sale_price'] *
                Object.values(myData[i])[0]['order_qty'] *
                1.2;
          } else {
            price =
              price +
  ...
  function updateVATstatus(status) {
    cache.store('currentVATstatus', status);
    if (status == '1') {
      setVATstatus('true');
    } else {
      setVATstatus('false');
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              <Text style={{fontSize: 17, paddingHorizontal: 10}}>
                You are about to place the order, you will not able to edit this
                order.
              </Text>
              <View
                style={{
                  ...
                  <View key={generateRandString()}>
                    {Object.values(value).map((val, k) => {
                      ...
                      return (
                        <View
                          style={
                            win.width > 500 ? styles.mainBoxTab : styles.mainBox
                          }
                          ...
                          <View
                            ...
                            <Image
                              source={{uri: imagePrefix + '' + val?.img}}
                              style={
                                win.width > 500
                                  ? {width: 50, height: 55, marginRight: 8}
                                  : {width: 40, height: 45, marginRight: 8}
                              }
                            />
                            <View key={generateRandString()}>
                              <Text
                          ...
                          <View
                            ...
                            <Text
                              ...
                              {(valuetem * val?.sale_price).toFixed(2)}
                            </Text>
                            ...
                            <Text
                              ...
                              {val.VATstatus == true ? (
                                <View>
                                  <Text>
                                    {(
                                      valuetem * val?.sale_price * 1.2 -
                                      valuetem * val?.sale_price
                                    ).toFixed(2)}
                                  </Text>
                                </View>
                              ) : (
